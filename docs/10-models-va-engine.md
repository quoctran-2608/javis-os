...
- **Trang Kết nối hiện dòng vàng "⚠ Main Model đang là ... - chưa hỗ trợ gọi công cụ. Đổi ở trang Models."**: dòng này chỉ dành cho provider lạ chưa có adapter tool. Mọi provider có sẵn đều gọi được MCP. ChatGPT OAuth hiện thẻ xanh cho đường Codex CLI; Google Antigravity hiện thẻ xanh cho đường agent CLI + cầu MCP stdio; các provider API hiện thẻ xanh cho vòng gọi tool qua hub.
